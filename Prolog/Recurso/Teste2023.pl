...
same_saga(Movie1, Movie2):-
    % associa o filme1 e o filme2 à mesma saga
...
% se a sga do filme já existe também tem que ser atualizada
add_movie_to_saga(Saga, Movie, Year, Duration, Score, Director, Composer, Cast):-
    ...
    assertz(movie(Movie, Year, SagaID, Duration, Score, Director, Composer, Cast)). % adicionar um filme 
...
movies_from_saga(Saga, Movies):-
    ...
    movies_from_saga(SagaID, [], Movies). % encontrar os filmes que pertencem à mesma saga
...
% 6
% retorna em Cast uma lista de actores que participaram nos filmes de saga sem duplicar
saga_cast(Saga, Cast):-
    saga(SagaID, Saga, _N, _Cr),
    %concontra um actor que pertence a um filme cuja sagaID é igual
    findall(Actor, (movie(_T, _Y, SagaID, _Dur, _Sc, _Dir, _Comp, Cast), member(Actor, Cast)), List),
    sort(List, Cast). % ordena a lista

% 7 
% o mesmo que a função anterior, mas só os atores em posições impares
sample_cast(Saga, Cast):-
    saga_cast(Saga, Cast), % para obter os atores que pertencem a uma saga
    sample(Cast, sampleCast).

sample([], []). % se não houver atores retorna uma lista vazia
sample([E], [E]). % se só houver retorna 1
sample([A,_B|T], [A|T2]):-
    sample(T, T2). % passa o segundo à frente e faz o mesmo

% 8
% para quantos filmes é que o compositor compôs música 
...
% retorna o grau de separação entre as duas pessoas (numero minimo de conecções num grafo)
